    ...
    public partial class Form1 : Form
    {
        ...
        private void Build_up_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Ro = Convert.ToDouble(textBoxRo.Text);

                HalfLife = Convert.ToDouble(textBoxHalfLife.Text);
                Nu = Convert.ToDouble(textBoxNu.Text);
        ...
        void method_of_EulerX()//рисует интегральную кривую и выводит n_max
        {
            ...
            {
                x1 = x;
                //x = x + Tau * (Alpha * x * y + Ro);
                //y = y + Tau * (Beta * x1 - HalfLife) * (Sigma * y + Fi * x1);
                x = x + Tau * (Alpha * (x * x) + (Beta * y * y) + Epsilon);
                y = x + Tau * (Sigma * x1 + Ro) * (HalfLife * x1 + Nu * y + Fi);
                list1.Add(x, y);
            }
            ...
            for (int par = 0; par < n; par++)
            {
                ...
                x = x - Tau * (Alpha * (x * x) + (Beta * y * y) + Epsilon);
                y = y - Tau * (Sigma * x1 + Ro) * (HalfLife * x1 + Nu * y + Fi);
                list2.Add(x, y);
            }
